...
#This file takes an input .vcf file and produces two output files. Both are tab-delimited tables.
#The first is a table of locus positions that are entirely invariant across all samples.
#The second is a table of locus positions that are only variant in one sample.
#Files are saved to the directory of the input .vcf file, and can then be used with vcftools to remove the unwanted loci.

#A .vcf file produced by whichever variant-calling software may contain sites where all individuals have the same genotype. They will still be considered variant, however, if any allele in that genotype is non-reference.
#SNP-based Bayesian softwares also use only variant sites, so you may want to remove completely homogeneous ones ahead of time for that purposes as well.
#Singleton loci can also be good to ID ahead of time.
...
print(paste("Total time taken for ",nrow(geno)," sites was ",round(Sys.time() - start.time,digits=2)," seconds."))
